...
syntax keyword DatacomContext mpls ospf router aaa rsvp snmp vlan ipv4 ipv6 sntp vrf l3 cpu arp icmp ssh ipv6-icmp vlan-id mpls-te ospfv3
...
syntax keyword DatacomCommand traffic-eng  attribute-set path-option explicit-path destination vacm access-list access-list-entry switchport counters
...
syntax keyword DatacomProperty source-port ethertype native-vlan ingress egress
syntax match DatacomProperty "\v((read|write|notify)-view)"
...
syntax match DatacomInterface "\v(((ten|hundred)-)?gigabit-ethernet|mgmt|loopback|(lag(-\d{1,2})?)|tunnel-te(-\d{1,2})?)"
syntax match DatacomInterface "\v(1/1/\d{1,3})"
" IPv4
syntax match DatacomInterface "\v(\d{1,3}\.){3}\d{1,3}"
" IPv6
...
syntax keyword DatacomValue local both md5 dynamic static point-to-point pw-status tagged untagged
